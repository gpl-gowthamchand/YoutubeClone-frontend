          ...
          searchResults.map((e, index) => {
            return (
              <div key={index * 2} className='flex flex-col gap-y-3 sm:flex-row gap-x-4 md:gap-x-8 w-[98%] justify-center cursor-pointer sm:mx-0'>
                <img onClick={() => pageRoute(`/watch/${e.id?.videoId}`)} className='w-[97%] sm:w-[29%] md:w-[25%] sm:rounded-[23px]' src={e.snippet?.thumbnails?.medium?.url} alt={e.snippet?.title || "Search result thumbnail"} />
